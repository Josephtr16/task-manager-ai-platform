...
class EmailService {
    ...
    async sendPasswordResetEmail(email, token) {
        ...
        try {
            return await this.sendEmail({
                email,
                subject: 'Password Reset - TaskFlow AI',
                message,
                html,
            });
        } catch (error) {
            console.warn('---------------------------------------------------------');
            console.warn('⚠️ PASSWORD RESET EMAIL FAILED (SMTP not configured?)');
            console.warn('📩 FOR TESTING, USE THIS LINK TO RESET PASSWORD:');
