...
import { FaArrowRight, FaCheckCircle, FaEnvelope, FaExclamationTriangle } from 'react-icons/fa';
...
const Login = () => {
  ...
  const pillItems = [
    { label: 'AI Prioritization', icon: 'spark' },
    { label: 'Kanban', icon: 'board' },
  ...
  return (
    <div className={`auth-login-page${isDarkMode ? ' dark' : ''}`} style={{ backgroundColor: pageBackgroundColor, backgroundImage: 'none' }}>
      ...
      <div className={`auth-login-shell${isDarkMode ? ' dark' : ''}`}>
        <section className="auth-login-left">
          ...
          <div className="login-copy">
            <h1>{tt('auth.welcomeBack', 'Sign in')}</h1>
            <p>{tt('authExtras.signInPrompt', 'Welcome back — sign in to your workspace.')}</p>
          </div>
            ...
            <div className="auth-alert auth-alert-error">
              ...
              <div className="auth-alert-body">
                ...
                {error.includes('verify') && (
                  <button type="button" onClick={handleResendEmail} className="auth-resend-link" disabled={resending}>
          ...
          {successMessage && (
            <div className="auth-alert auth-alert-success">
              <div className="auth-alert-icon"><FaCheckCircle size={18} /></div>
              <p>{successMessage}</p>
            </div>
          )}
          ...
          {resendSuccess && (
            <div className="auth-alert auth-alert-success">
              <div className="auth-alert-icon"><FaEnvelope size={18} /></div>
              <p>{resendSuccess}</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="auth-form">
            ...
            <div className="field-group">
              <div className="password-meta-row">
                <label htmlFor="password" className="field-label">{tt('auth.password', 'PASSWORD')}</label>
                <Link to="/forgot-password" className="forgot-link">{tt('authExtras.forgotShort', 'Forgot?')}</Link>
              </div>
              <div className="field-input-wrap">
                <span className="field-icon field-icon-left" aria-hidden="true">
                  <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
                ...
                <input
                  ...
                  className="auth-input auth-input-password" required
                />
            ...
            <button type="submit" className={`login-submit-btn${loading ? ' is-loading' : ''}`} disabled={loading}>
              {loading ? (
                <><span className="spinner" />{tt('authExtras.signingIn', 'Signing in...')}</>
              ) : (
                <>{tt('authExtras.signIn', 'Sign in')}<FaArrowRight size={15} /></>
              )}
            </button>
          </form>

          <div className="footer-row">
            <p>{tt('authExtras.noAccount', 'No account?')}{' '}<Link to="/register">{tt('auth.signUp', 'Sign up')}</Link></p>
          </div>
        </section>
        ...
        <section className="auth-login-right">
          <div className="right-block">
            ...
            <p>AI-driven prioritization, smart time estimates, Kanban, Calendar, and productivity insights — all in one warm workspace built for focus.</p>

            <div className="feature-pills">
              {pillItems.map((pill) => (
                <span className="feature-pill" key={pill.label}>
                  <svg viewBox="0 0 16 16" focusable="false" aria-hidden="true" className="pill-icon">
                    {pill.icon === 'spark' && <path d="M8 1.4 9.6 5l3.6 1.6-3.6 1.6L8 11.8 6.4 8.2 2.8 6.6 6.4 5 8 1.4Z" />}
                    {pill.icon === 'board' && <path d="M3 3h10a1 1 0 0 1 1 1v8H2V4a1 1 0 0 1 1-1Zm0 2v6h9V5H3Zm1 1h3v1H4V6Zm0 2h5v1H4V8Z" />}
                    {pill.icon === 'chart' && <path d="M3 12h10v1H2V3h1v9Zm2-1V7h1v4H5Zm3 0V5h1v6H8Zm3 0V8h1v3h-1Z" />}
                    {pill.icon === 'calendar' && <path d="M4 3h1V2h1v1h4V2h1v1h1.5A1.5 1.5 0 0 1 14 4.5v8A1.5 1.5 0 0 1 12.5 14h-9A1.5 1.5 0 0 1 2 12.5v-8A1.5 1.5 0 0 1 3.5 3H4Zm9 4H3v5.5c0 .3.2.5.5.5h9a.5.5 0 0 0 .5-.5V7Z" />}
                    {pill.icon === 'timer' && <path d="M7 2h2v1.2a5.5 5.5 0 1 1-2 0V2Zm1 3a4.5 4.5 0 1 0 0 9 4.5 4.5 0 0 0 0-9Zm-.5 1.5h1v2.7l1.9 1.1-.5.9-2.4-1.4V6.5Z" />}
                  </svg>
      ...
      <style>{`
        ...
        .password-meta-row { display: flex; align-items: center; justify-content: space-between; }

        .forgot-link { color: #B07A2A; font-size: 12px; font-weight: 600; text-decoration: none; }
        ...
        .footer-row { margin-top: 22px; font-size: 13px; color: #8A8070; }
        ...
        .right-block p { margin: 0 0 20px 0; max-width: 420px; font-size: 15px; line-height: 1.75; color: #6B5E4A; }
        ...
        .dark.auth-login-page .forgot-link,
        .dark.auth-login-page .footer-row a,
        .dark.auth-login-page .auth-resend-link { color: #C9924A; }
        ...
        .dark.auth-login-page .auth-alert-success { background: #1C1A18; border-color: rgba(255, 248, 235, 0.06); }
        ...
        @media (max-width: 1200px) {
          .auth-login-left { padding: 0 6% 0 6%; padding-top: 100px; }
